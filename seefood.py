...
def label_image(img, confidence=50):
    ...
    imagebytes = image_helpers.get_image(img)
    response = client.detect_labels(Image={'Bytes':imagebytes})
    labels = response['Labels']
    if any(label['Name'] == 'Hot Dog' for label in labels):
        output_label = 'Hot Dog'
    else:
        output_label = 'Not Hot Dog'
    img = get_pillow_img(imagebytes)
    imgoutput = add_text_to_img(img,output_label)
    return imgoutput
